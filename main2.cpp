...
//    auto matrix_arr = {"mats/nos3.mtx", "mats/plbuckle.mtx", "mats/bcsstk08.mtx", "mats/bcsstk09.mtx", "mats/G51.mtx", "mats/bcsstm13.mtx", "mats/gemat11.mtx"};
    ...
    out << "numOfColor,mnat,mnew,mlfo,msat,k,ck" << endl;
...
//#pragma omp parallel for
        for (int k = 0; k < 100; k += 10) {
            ...
            for (int color = from; color <= to; color += step) {
                auto[all_sum_nat, all_discovered_color_zero_sum_nat, all_misses_nat, all_misses_color_zero_sum_nat] = compute_discovered_misses(
                        color_vec_natural, m, color);
                auto[all_sum_new, all_discovered_color_zero_sum_new, all_misses_new, all_misses_color_zero_sum_new] = compute_discovered_misses(
                        color_vec_newIdea, m, color);
                auto[all_sum_lfo, all_discovered_color_zero_sum_lfo, all_misses_lfo, all_misses_color_zero_sum_lfo] = compute_discovered_misses(
                        color_vec_lfo, m, color);
                auto[all_sum_sat, all_discovered_color_zero_sum_sat, all_misses_sat, all_misses_color_zero_sum_sat] = compute_discovered_misses(
                        color_vec_sat, m, color);
                ...
                int all_discovered_sat =
                        all_sum_sat + all_discovered_color_zero_sum_sat;//compute_discovered(color_vec_sat, m, color);
                out << color << "," << all_discovered_natural << "," << all_discovered_newIdea << ","
                    << all_discovered_lfo << "," << all_discovered_sat << "," << k << "," << num_colors_natural_full
                    << endl;
            }
        }
    }
